	...
	for (var i = 0; i < json.data.museum.exhibits.length; i++) {
		...
		for (var j = 0; j < exhibit.components.length; j++) {
			...
			componentModel.set({
				'id' : component.id,
				'name' : component.name,
				"compOrderNo" : component.order_number,
				'exhibit' : exhibit.name,
				"exhibOrderNo" : exhibit.order_number
			});
...
function createComponentsScrollView(exhibits) {
	currExhibitId = exhibits[0].order_number;

	for (var i = 0; i < exhibits.length; i++) {

		componentsInExhibit[exhibits[i].order_number] = Ti.UI.createView({
			layout : 'horizontal',
			horizontalWrap : false,
			width : Ti.UI.SIZE
		});

		// componentsInExhibit.sort(function(a,b){
		// return a.order_number > b.order_number;
		// });
		...
		for (var j = 0; j < exhibits[i].components.length; j++) {
			...
			componentsInExhibit[exhibits[i].order_number].add(component);
		}
		$.componentScrollView.add(componentsInExhibit[exhibits[i].order_number]);
		componentsInExhibit[exhibits[i].order_number].width = 0;
		Ti.API.info(componentsInExhibit);
	}

	componentsInExhibit[currExhibitId].width = Ti.UI.SIZE;
	Ti.API.info("---------------------");
	Ti.API.info(JSON.stringify(componentsInExhibit));
	Ti.API.info(componentsInExhibit);
	Ti.API.info(componentsInExhibit[0]);
	Ti.API.info(JSON.stringify(componentsInExhibit[0]));
}
